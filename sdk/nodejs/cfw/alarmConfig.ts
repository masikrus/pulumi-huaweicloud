 ...
 * const alarmType = config.requireObject("alarmType");
 * const alarmTimePeriod = config.requireObject("alarmTimePeriod");
 * const severity = config.requireObject("severity");
 * const frequencyCount = config.requireObject("frequencyCount");
 * const frequencyTime = config.requireObject("frequencyTime");
 * const topicUrn = config.requireObject("topicUrn");
 * const test = new huaweicloud.cfw.AlarmConfig("test", {
 ...
 * ## Import
 *
 * The alarm configuration can be imported using `fw_instance_id`, `alarm_type`, separated by a slash, e.g. bash
 *
 * ```sh
...
export class AlarmConfig extends pulumi.CustomResource {
    ...
    /** @internal */
    public static readonly __pulumiType = 'huaweicloud:Cfw/alarmConfig:AlarmConfig';
