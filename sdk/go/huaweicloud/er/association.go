...
// ## Import
//
// Associations can be imported using their `id` and the related `instance_id` and `route_table_id`, separated by slashes (/), e.g. bash
//
// ```sh
//
...
func NewAssociation(ctx *pulumi.Context,
	...
	err := ctx.RegisterResource("huaweicloud:Er/association:Association", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetAssociation(ctx *pulumi.Context,
	...
	var resource Association
	err := ctx.ReadResource("huaweicloud:Er/association:Association", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
