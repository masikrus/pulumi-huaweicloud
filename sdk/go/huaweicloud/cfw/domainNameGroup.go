...
// ## Import
//
// The domainnamegroup can be imported using the `fw_instance_id`, `object_id` and `id`, separated by slashes, e.g. bash
//
// ```sh
//
...
func NewDomainNameGroup(ctx *pulumi.Context,
	...
	err := ctx.RegisterResource("huaweicloud:Cfw/domainNameGroup:DomainNameGroup", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetDomainNameGroup(ctx *pulumi.Context,
	...
	var resource DomainNameGroup
	err := ctx.ReadResource("huaweicloud:Cfw/domainNameGroup:DomainNameGroup", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
