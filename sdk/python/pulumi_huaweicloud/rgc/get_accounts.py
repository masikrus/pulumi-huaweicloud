# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs

__all__ = [
    'GetAccountsResult',
    'AwaitableGetAccountsResult',
    'get_accounts',
    'get_accounts_output',
]

@pulumi.output_type
class GetAccountsResult:
    """
    A collection of values returned by getAccounts.
    """
    def __init__(__self__, control_id=None, id=None, managed_accounts=None, region=None):
        if control_id and not isinstance(control_id, str):
            raise TypeError("Expected argument 'control_id' to be a str")
        pulumi.set(__self__, "control_id", control_id)
        if id and not isinstance(id, str):
            raise TypeError("Expected argument 'id' to be a str")
        pulumi.set(__self__, "id", id)
        if managed_accounts and not isinstance(managed_accounts, list):
            raise TypeError("Expected argument 'managed_accounts' to be a list")
        pulumi.set(__self__, "managed_accounts", managed_accounts)
        if region and not isinstance(region, str):
            raise TypeError("Expected argument 'region' to be a str")
        pulumi.set(__self__, "region", region)

    @_builtins.property
    @pulumi.getter(name="controlId")
    def control_id(self) -> Optional[_builtins.str]:
        return pulumi.get(self, "control_id")

    @_builtins.property
    @pulumi.getter
    def id(self) -> _builtins.str:
        """
        The provider-assigned unique ID for this managed resource.
        """
        return pulumi.get(self, "id")

    @_builtins.property
    @pulumi.getter(name="managedAccounts")
    def managed_accounts(self) -> Sequence['outputs.GetAccountsManagedAccountResult']:
        """
        The managed account information.
        """
        return pulumi.get(self, "managed_accounts")

    @_builtins.property
    @pulumi.getter
    def region(self) -> _builtins.str:
        """
        The region name.
        """
        return pulumi.get(self, "region")


class AwaitableGetAccountsResult(GetAccountsResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetAccountsResult(
            control_id=self.control_id,
            id=self.id,
            managed_accounts=self.managed_accounts,
            region=self.region)


def get_accounts(control_id: Optional[_builtins.str] = None,
                 region: Optional[_builtins.str] = None,
                 opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetAccountsResult:
    """
    Use this data source to list managed accounts in Resource Governance Center.

    ## Example Usage

    ```python
    import pulumi
    import pulumi_huaweicloud as huaweicloud

    config = pulumi.Config()
    control_id = config.require_object("controlId")
    test = huaweicloud.Rgc.get_accounts()
    test_control_id = huaweicloud.Rgc.get_accounts(control_id=control_id)
    ```


    :param _builtins.str control_id: The enabled control policy information.
    :param _builtins.str region: Specifies the region in which to create the resource.
           If omitted, the provider-level region will be used. Changing this parameter will create a new resource.
    """
    __args__ = dict()
    __args__['controlId'] = control_id
    __args__['region'] = region
    opts = pulumi.InvokeOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke('huaweicloud:Rgc/getAccounts:getAccounts', __args__, opts=opts, typ=GetAccountsResult).value

    return AwaitableGetAccountsResult(
        control_id=pulumi.get(__ret__, 'control_id'),
        id=pulumi.get(__ret__, 'id'),
        managed_accounts=pulumi.get(__ret__, 'managed_accounts'),
        region=pulumi.get(__ret__, 'region'))
def get_accounts_output(control_id: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                        region: Optional[pulumi.Input[Optional[_builtins.str]]] = None,
                        opts: Optional[Union[pulumi.InvokeOptions, pulumi.InvokeOutputOptions]] = None) -> pulumi.Output[GetAccountsResult]:
    """
    Use this data source to list managed accounts in Resource Governance Center.

    ## Example Usage

    ```python
    import pulumi
    import pulumi_huaweicloud as huaweicloud

    config = pulumi.Config()
    control_id = config.require_object("controlId")
    test = huaweicloud.Rgc.get_accounts()
    test_control_id = huaweicloud.Rgc.get_accounts(control_id=control_id)
    ```


    :param _builtins.str control_id: The enabled control policy information.
    :param _builtins.str region: Specifies the region in which to create the resource.
           If omitted, the provider-level region will be used. Changing this parameter will create a new resource.
    """
    __args__ = dict()
    __args__['controlId'] = control_id
    __args__['region'] = region
    opts = pulumi.InvokeOutputOptions.merge(_utilities.get_invoke_opts_defaults(), opts)
    __ret__ = pulumi.runtime.invoke_output('huaweicloud:Rgc/getAccounts:getAccounts', __args__, opts=opts, typ=GetAccountsResult)
    return __ret__.apply(lambda __response__: GetAccountsResult(
        control_id=pulumi.get(__response__, 'control_id'),
        id=pulumi.get(__response__, 'id'),
        managed_accounts=pulumi.get(__response__, 'managed_accounts'),
        region=pulumi.get(__response__, 'region')))
